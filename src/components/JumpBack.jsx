...
const JumpBack = () => {
  ...
  useEffect(() =>{
    setIsloading(true)
  if (token) {
    fetch(`${import.meta.env.VITE_API_TOP_MIX}browse/categories/0JQ5DAqbMKFLVaM30PMBm4/playlists`, {
      method: "GET",
      headers:{
...
<h2 className='text-white font-semibold text-2xl'>Jump Back In</h2>
<button className='text-white opacity-40'>SEE ALL</button>
     ...
     <div className="card-wrapper w-full flex gap-4 flex-wrap justify-center">
      ...
      isLoading ? <RingLoader color="#8136d7"height={100} width={10} className='mt-16 w-96'/> : topMix.length > 0 && topMix.slice(0, 4).map((el, index) =>{
        return (
      <div key={index} className="card w-[200px] object-cover cursor-pointer hover:shadow-2xl duration-300 bg-[#1b1b1b] p-5 rounded-md" >
